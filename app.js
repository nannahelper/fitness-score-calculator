...
function calculate2400(){if(!data||!$('#runResult'))return;const seconds=parseTime('runMin','runSec'), gender=$('#runGender').value; if(seconds==null){$('#runResult').innerHTML='<div class="empty-state">输入分钟和秒</div>';return}const rows=dataset().legacyStandard['2400m'][gender];let found=rows.find(r=>seconds<=r.seconds);const outOfRange=!found && rows[rows.length-1].score>0;if(!found)found=rows[rows.length-1];const score=seconds<rows[0].seconds?100:outOfRange?null:found.score;const note=outOfRange?'低于本版最低 50 分档（表外）':score===100&&seconds<rows[0].seconds?'超过满分档位':score===0?'低于最低档位':`对应档位 ${displayTime(found.seconds)}`;$('#runResult').innerHTML=`<div class="single-score"><div class="number">${score==null?'—':score.toFixed(1)}</div><p>${score==null?'表外':scoreLevel(score)} · ${note} · 实测 ${displayTime(seconds)}</p></div>`}
...
function updateVersionUI(){const spring=activeVersion==='springAutumn';$('#versionSpring').classList.toggle('active',spring);$('#versionLegacy').classList.toggle('active',!spring);$('#versionNote').textContent=spring?'春秋版默认：秋季综合表与旧版完全一致；2400 米男生满分档由 10:00 改为 10:30，男女新表均列到 50 分；另有春季力量对照表。':'旧版：2400 米覆盖 0–100 分（男 10:00–18:00、女 13:00–21:00）；综合表与春秋版秋季部分一致。'}
